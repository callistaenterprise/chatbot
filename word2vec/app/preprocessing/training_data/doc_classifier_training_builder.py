...
class DocClassifierTrainingBuilder(TrainingDataBuilder):
    ...
    def _document_to_paragraphs(self, doc):
        ...
        with open(doc, "r", encoding="utf-8", errors="ignore") as doc_data:
            for line in doc_data:
                doc_as_line = " ".join(doc_as_line, line.strip())
        ...
        for paragraph in self._paragraphize(words):
            ...
            padded_paragraph = word_ids.expand(buffer[len(word_ids):self.paragraph_length])
            paragraphs.append(padded_paragraph)
        return paragraphs

    def _doc_to_paragraph_ids(self, doc_filename, paragraphs):
        paragraph_ids = list(range(self._paragraph_id, len(paragraphs)))
        self._document_to_paragraphs(doc_filename, paragraph_ids)
    ...
    def _build_samples_for_paragraph(self, paragraph, paragraph_id):
        ...
        for focus_word_pos, focus_word_id in enumerate(paragraph):
            ...
            # [paragraph_id, context_word1_id, context_word2_id, ... context_word<window_size*2>_id]
    ...
    def build_training_data(self):
        ...
        for doc in self.docs:
            ...
            self._doc_to_paragraph_ids[doc_filename, doc_paragraphs]
            for (paragraph_id, paragraph) in zip(self._doc_to_paragraph_ids[doc_filename], doc_paragraphs):
...
def main():
    ...
    classifier_training_builder = DocClassifierTrainingBuilder(source_dir, paragraph_length, vector_size, window_size,  tokenizer_file)
