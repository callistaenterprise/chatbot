...
class TrainingDataBuilder(object):
    ...
    def __init__(self, cleaned_file, window_size, dry_run=False):
        ...
        self.training_data_file = path.join(
            dir_name, "../../data/4_training_data/cbow_training_data.dat"
        )
        self.sg_training_data_file = path.join(
            dir_name, "../../data/4_training_data/sg_training_data.dat"
        )
        self.glove_training_data_file = path.join(
            dir_name, "../../data/4_training_data/glove_training_data.dat"
        )
    ...
    def build_cbow_training_data(self):
        ...
        if path.exists(self.training_data_file) and self.dry_run is False:
            X_y = load_training_data(self.training_data_file)
        else:
            X_y = dict()
            X = []
            y = []
